...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: "Transcreva o áudio para português brasileiro. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida em parágrafos quando for apropriado.",
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
        ...
        INSTRUÇÕES:
        - Use apenas as informações contidas no contexto enviado;
        ...
        - Cite trechos importantes do contexto se apropriado;
